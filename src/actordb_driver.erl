...
-export([init/1,noop/1,
         ...
         parse_helper/1,parse_helper/2, iterate_wal/1,page_size/0, %wal_pages/1,
...
iterate_wal({actordb_driver, _Ref, Connection}) ->
    Ref = make_ref(),
    ok = actordb_driver_nif:iterate_wal(Connection, Ref, self()),
    receive_answer(Ref).
