...
**    32: prevFrameOffset (ActorDB addition: index of previous frame in wal file for actor)
**    36: Actorname (ActorDB addition)
...
static void walEncodeFrame(
  ...
  if (conn)
  {
    ...
    sqlite3Put4byte(&aFrame[32], pWal->prevFrameOffset);
    memcpy((char*)&aFrame[36],conn->dbpath,MAX_ACTOR_NAME);
  }
  else
  {
    memset((void *)&aFrame[8], 0, 120);
  }
  // DBG((g_log,"Writing %d npage %d, writing truncate %d\r\n",pWal->hdr.bigEndCksum,iPage,nTruncate));
  // DBG((g_log,"Writing filename %s, actorindex %d, wn %llu, wtn %llu\r\n",conn->dbpath,conn->connindex,conn->writeNumber,conn->writeTermNumber));
  // DBG((g_log,"Writing salt %d %d\r\n",pWal->hdr.aSalt[0],pWal->hdr.aSalt[1]));
...
static int walDecodeFrame(
  ...
  *actorIndex = sqlite3Get4byte(&aFrame[24]);
  *threadWriteNum = sqlite3Get4byte(&aFrame[28]);
  *prevFrameOffset = sqlite3Get4byte(&aFrame[32]);
  if (filename != NULL)
  	memcpy(filename,&aFrame[36],MAX_ACTOR_NAME);
...
int checkpoint_continue(db_thread *thread)
{
	...
	// This way we have more space for replication (raft).
	...
	for (i = wFile->checkpointPos; i < thread->nconns; i++, wFile->checkpointPos++)
	{
		...
		else if (curConn->nErlOpen == 0 && curConn->wal->prev == NULL && curConn->wal->hdr.mxFrame == 0)
		{
	        ...
	        if(rc != SQLITE_OK)
	        {
	            DBG((g_log,"Unable to close DB from checkpoint\r\n"));
	            return -1;
	        }
	        else
	        {
  ...
  while(rc == SQLITE_OK)
  {
    ...
    if (nSize > WAL_HDRSIZE)
    {
      ...
      for(iOffset=WAL_HDRSIZE; (iOffset+szFrame)<=nSize; iOffset+=szFrame)
      {
        ...
        // DBG((g_log,"Reading wal %llu, frame %d\r\n",curWal->walIndex,iFrame));
        ...
        // DBG((g_log,"Frame belongs to %s, truncate %d, pgno %d\r\n",filename,nTruncate,pgno));
        ...
        else
        {
          ...
          if (curConn->wal == NULL)
          {
		    ...
		    curConn->wal->szPage = curWal->szPage;
		    thread->curConn = curConn;
...
int wal_rewind(db_connection *conn, u64 evnum)
{
	...
	while (wal != NULL)
	{
		...
		for(iOffset=WAL_HDRSIZE; (iOffset+szFrame)<=nSize; iOffset+=szFrame)
		{
	        ...
	        // DBG((g_log,"Index append wal=%llu, frame=%d,offset=%d, pgno=%d, evnum=%llu, trunc=%d\r\n",
			...
			sqlite3Put4byte(&buffer[32], prevFrameOffset);
...
int wal_iterate_from(db_connection *conn, iterate_resource *iter, int bufSize, u8* buffer, 
	int *nFilled,		// OUT: how many bytes written to buffer
	char *activeWal) 	// OUT: are we reading active wal file
{
	...
	*activeWal = 0;
	...
	// iterate over all frames in wal for connection. Start with oldest wal file.
	else if (iter->evnumFrom == 0)
	{
		...
		while (!found)
		{
	    	...
	    	{
	    		// if first wal is current, we have not found any frames in wal for connection
	    		if (conn->wal == wal)
	    			return SQLITE_DONE;
	...
	// iterate from some evnum on. Start from the end and move back until reaching evnum.
	else
	{
		// Find first frame of evnum.
		while (wal != NULL && !found)
		{
			wal->hdr.aSalt[0] = 123456789;
			wal->hdr.aSalt[1] = 987654321;
	...
	wal = conn->wal;
	while (wal != NULL && wal->walIndex != iter->walIndex)
		wal = wal->prev;
	...
	{
		rc = sqlite3OsFileSize(wal->pWalFd, &nSize);
		for(iOffset = iter->iOffset; (iOffset+szFrame)<=nSize; iOffset+=szFrame)
		{
			rc = sqlite3OsRead(wal->pWalFd, buffer+(*nFilled), szFrame, iOffset);
	        if( rc!=SQLITE_OK )
	        {
	        	break;
	        }
...
int sqlite3WalOpen(
	...
	while (walFile != NULL && walFile->walIndex != curWal->walIndex)
...
int sqlite3WalCheckpoint(
  ...
  while (pWal->prev != NULL && pWal->walIndex != wFile->walIndex)
  	pWal = pWal->prev;
...
//  }
  ...
  else if (rc == SQLITE_OK && pWalTop == pWal)
  {
